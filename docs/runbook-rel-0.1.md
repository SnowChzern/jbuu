# jbuu（锦书）部署 runbook（REL-0.1）

| 项 | 内容 |
|---|---|
| 适用 | tag `v0.1.1`（= v0.1 基线 `fb4a192` + 任务 #62 改名 patch；commit 见交付帖）release 工件：`jbuu` 0.1.1 + `jbuu-doorkeeper` 0.1.1（x86_64-unknown-linux-gnu）。v0.1 工件与 tag `v0.1` 不动不重打，历史口径见该时点文档 |
| 任务 | 论坛任务 #61 · [jbuu][REL-0.1] v0.1 发布工件：release 构建 + 部署 runbook；v0.1.1 改名同步：论坛任务 #62 · [jbuu][REL-0.2] 锦书改名：二进制 otp-term→jbuu |
...
| 边界 | 部署执行归芥末（楼295 先例），本文只到步骤；GitHub 公开仓同步归调度，本文不涉及；锦书改名已由任务 #62 落地：v0.1.1 起二进制名 `jbuu`（v0.1 工件与 tag 维持旧名不重打） |

## 0. 口径与前提

### 0.1 安全口径（先读，不可弱化）

1. **jbuu 服务面只走内网 mesh VPN**：`serve` 监听地址必须是 mesh 内网地址；**禁止**绑定 `0.0.0.0`/`[::]`，禁止任何公网端口映射/ DNAT 指向 serve 端口。客户端同样只在 mesh 内发起连接。
...
3. doorkeeper 与 jbuu 服务面**互不相干**：§4 全程不动 mesh/serve；§2–§3 全程不动 sshd/doorkeeper。两块可独立执行、独立回滚。
...
SRV_PORT=7717               # jbuu serve 监听端口（mesh 内自定，避开 22/2222/2223）
...
```sh
cd /path/to/rel-0.1.1                     # 工件目录（含 jbuu、jbuu-doorkeeper、*.service、SHA256SUMS）
sha256sum -c SHA256SUMS                   # 期望：每行“成功”
./jbuu --version                      # 期望：jbuu 0.1.1
./jbuu-doorkeeper --help | head -1        # 期望：安全边界声明开头
```

## 2. server 侧：安装与初始化（jbuu 服务面）

### 2.1 目录与二进制落位

```sh
sudo install -d -m 0755 /opt/jbuu/v0.1.1
sudo install -m 0755 jbuu /opt/jbuu/v0.1.1/jbuu
...
```sh
sudo /opt/jbuu/v0.1.1/jbuu book generate /var/lib/jbuu/otp.book --segments 1024
...
```sh
BOOK_ID=$(/opt/jbuu/v0.1.1/jbuu book inspect /var/lib/jbuu/otp.book --json \
...
PY
sudo /opt/jbuu/v0.1.1/jbuu anchor inspect /var/lib/jbuu/anchor-a.anchor /var/lib/jbuu/anchor-b.anchor
...
```sh
sudo /opt/jbuu/v0.1.1/jbuu doctor --book /var/lib/jbuu/otp.book \
...
sudo tee /etc/systemd/system/jbuu-serve.service >/dev/null <<'UNIT'
[Unit]
Description=jbuu v0.1.1 serve - OTP book terminal service (mesh only)
...
ExecStart=/opt/jbuu/v0.1.1/jbuu serve \
...
sudo systemctl enable --now jbuu-serve.service
```
...
```sh
systemctl is-active jbuu-serve.service                      # 期望 active
sudo journalctl -u jbuu-serve.service -n 20 --no-pager      # 期望含 READY 与 LISTEN= 行
...
BOOK_ID=$(./jbuu book inspect otp.book --json | grep -o '"book_id": *"[0-9a-f]\{32\}"' | grep -o '[0-9a-f]\{32\}')
...
PY
printf 'ops marker\n' > .jbuu-nobackup
./jbuu anchor inspect anchor-a.anchor anchor-b.anchor        # 期望：一致/Init/generation=0/next=0
./jbuu doctor --book otp.book --anchor-a anchor-a.anchor --anchor-b anchor-b.anchor
...
./jbuu connect --book otp.book --anchor-a anchor-a.anchor --anchor-b anchor-b.anchor \
...
./jbuu doctor --book ... --anchor-a ... --anchor-b ... --json > doctor-$(date -u +%Y%m%d).json
```
...
| backup-risk: book-dir | block | 放 marker `.jbuu-nobackup`（§2.4）或把目录移出备份域 |

## 7. 冒烟验收清单（上线前逐项打勾；v0.1 构建机已实测 11/11，v0.1.1 改名后等价复测证据见任务 #62 交付帖）
...
./jbuu book generate srv/otp.book --segments 64 | grep book_id        # ✅ 打印 book_id
BOOK_ID=$(./jbuu book inspect srv/otp.book --json | grep -o '"book_id": *"[0-9a-f]\{32\}"' | grep -o '[0-9a-f]\{32\}')
...
for p in ("$d/anchor-a.anchor","$d/anchor-b.anchor"):
    open(p,"wb").write(bytes(r)); os.chmod(p,0o600)
PY
printf 'ops marker\n' | tee "$d/.jbuu-nobackup" >/dev/null; done
# ② doctor 双侧无 BLOCK：
./jbuu doctor --book srv/otp.book --anchor-a srv/anchor-a.anchor --anchor-b srv/anchor-b.anchor --allow-unencrypted-swap
./jbuu doctor --book cli/otp.book --anchor-a cli/anchor-a.anchor --anchor-b cli/anchor-b.anchor --allow-unencrypted-swap
# ③ serve 起服务（2 个会话位）：
./jbuu serve --book srv/otp.book --anchor-a srv/anchor-a.anchor --anchor-b srv/anchor-b.anchor \
...
./jbuu connect --book cli/otp.book --anchor-a cli/anchor-a.anchor --anchor-b cli/anchor-b.anchor \
...
> 构建机等价自动化脚本与完整输出：`workspace/evidence/rel-0.1/smoke-run.sh`、`smoke-rel-0.1.log`（v0.1：11/11 PASS，含 SIGKILL 真断线证据：客户端无 DONE 行、rc=137）；v0.1.1 改名后等价复测：`workspace/evidence/task-62/`。
...
- 锦书更名：二进制名自 v0.1.1 起为 `jbuu`（任务 #62；v0.1 工件与 tag 不动不重打）；仓命名空间/GitHub 公开仓同步归调度另卡；备份排除 marker 主名同步改为 `.jbuu-nobackup`，v0.1 站点已放的 `.otp-term-nobackup` 仍被识别。
